...
from app.models_v2.schedule import (
    ScheduleV2Import, ScheduleV2Project, ScheduleV2WBS,
    ScheduleV2Calendar, ScheduleV2Resource,
    ScheduleV2Activity, ScheduleV2Relationship,
    ScheduleV2ResourceAssignment, ScheduleV2Dependency,
)
from app.models_v2.structure import ScheduleV2Corridor, ScheduleV2Package
from app.models_v2.contract_schedule import ScheduleV2Contract
...
async def _seed(s: AsyncSession) -> None:
    # 1. Import
    if not await _exists(s, ScheduleV2Import, IMPORT_01):
        s.add(ScheduleV2Import(
    ...
    # 2. Project
    if not await _exists(s, ScheduleV2Project, PROJ_01):
        s.add(ScheduleV2Project(
    ...
    for cal_id, clndr_id, name, day_hr, week_hr in [
        ...
        if not await _exists(s, ScheduleV2Calendar, cal_id):
            s.add(ScheduleV2Calendar(
    ...
    for wbs_id, wbs_int, name, short, parent_int, seq in wbs_rows:
        if not await _exists(s, ScheduleV2WBS, wbs_id):
            s.add(ScheduleV2WBS(
    ...
    for rsrc_id, rsrc_int, name, short, rtype, email in [
        ...
        if not await _exists(s, ScheduleV2Resource, rsrc_id):
            s.add(ScheduleV2Resource(
    ...
    for con_id, num, title, desc, std, val, eot in contracts:
        if not await _exists(s, ScheduleV2Contract, con_id):
            s.add(ScheduleV2Contract(
    ...
    for cor_id, name, code in [
        ...
        if not await _exists(s, ScheduleV2Corridor, cor_id):
            s.add(ScheduleV2Corridor(
    ...
    for pkg_id, name, code, cor_id, con_id, wbs_int, parent_pkg, subsystem in packages:
        if not await _exists(s, ScheduleV2Package, pkg_id):
            s.add(ScheduleV2Package(
    ...
    # 9. Activities
    # Note: clndr_id is an integer field (BigInteger) referencing clndr_id from ScheduleV2Calendar
    activities = [
    ...
    for (act_id, task_int, p6_id, name, status, atype, subsys, pkg_id,
        ...
        if not await _exists(s, ScheduleV2Activity, act_id):
            s.add(ScheduleV2Activity(
    ...
    for pred, succ, rtype, lag in rels:
        existing = await s.execute(
            select(ScheduleV2Relationship).where(
                ScheduleV2Relationship.pred_task_id == (
                    await s.execute(select(ScheduleV2Activity.task_id).where(ScheduleV2Activity.id == pred))
                ).scalar_one_or_none(),
                ScheduleV2Relationship.task_id == (
                    await s.execute(select(ScheduleV2Activity.task_id).where(ScheduleV2Activity.id == succ))
                ).scalar_one_or_none(),
            )
        )
        if not existing.scalars().first():
            pred_task_int = (await s.execute(select(ScheduleV2Activity.task_id).where(ScheduleV2Activity.id == pred))).scalar_one_or_none()
            succ_task_int = (await s.execute(select(ScheduleV2Activity.task_id).where(ScheduleV2Activity.id == succ))).scalar_one_or_none()
            if pred_task_int and succ_task_int:
                s.add(ScheduleV2Relationship(
    ...
    for act_id, rsrc_id, rsrc_int, tgt_qty, tgt_cost, act_qty, act_cost in assignments:
        ...
        if not await _exists(s, ScheduleV2ResourceAssignment, assign_uuid):
            act_int = (await s.execute(select(ScheduleV2Activity.task_id).where(ScheduleV2Activity.id == act_id))).scalar_one_or_none()
            rsrc_name_val = (await s.execute(select(ScheduleV2Resource.rsrc_name).where(ScheduleV2Resource.id == rsrc_id))).scalar_one_or_none()
            act_name_val = (await s.execute(select(ScheduleV2Activity.activity_name).where(ScheduleV2Activity.id == act_id))).scalar_one_or_none()
            act_code_val = (await s.execute(select(ScheduleV2Activity.p6_activity_id).where(ScheduleV2Activity.id == act_id))).scalar_one_or_none()
            if act_int:
                s.add(ScheduleV2ResourceAssignment(
    ...
    for pred_id, succ_id, dtype, lag, cross_pkg, cross_sys in dep_pairs:
        ...
        if not await _exists(s, ScheduleV2Dependency, dep_uuid):
            s.add(ScheduleV2Dependency(
