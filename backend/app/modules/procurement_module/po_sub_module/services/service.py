...
class POService:
    ...
    async def create(self, payload: POCreateDTO) -> ProcurementPO:
    ...
    async def get_by_id(self, po_id: UUID) -> ProcurementPO:
        po = await self.repo.get_by_id(po_id)
        if not po:
            raise ValueError(f"PO {po_id} not found")
        return po

    async def update(self, po_id: UUID, payload: POUpdateDTO) -> ProcurementPO:
    ...
    async def acknowledge(self, po_id: UUID) -> ProcurementPO:
        po = await self.get_by_id(po_id)
    ...
    async def dispatch(self, po_id: UUID) -> ProcurementPO:
        po = await self.get_by_id(po_id)
    ...
    async def close(self, po_id: UUID) -> ProcurementPO:
        po = await self.get_by_id(po_id)
    ...
    async def issue(self, po_id: UUID) -> ProcurementPO:
        po = await self.get_by_id(po_id)
    ...
    async def add_item(self, po_id: UUID, payload: POLineItemCreateDTO) -> ProcurementPOLineItem:
    ...
    async def update_item(self, po_id: UUID, item_id: UUID, payload: POLineItemUpdateDTO) -> ProcurementPOLineItem:
        await self.get_by_id(po_id)
